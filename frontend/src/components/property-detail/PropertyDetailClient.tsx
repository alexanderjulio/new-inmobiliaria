...
import { MapPin, Bed, Bath, Maximize, Phone, Mail, Share2, Heart } from 'lucide-react';
      ...
      <div className="container" style={{ marginTop: '2rem', display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
        <section>
          ...
          <div style={{ background: 'var(--surface-1)', padding: '1rem', borderRadius: 'var(--border-radius)', marginBottom: '2rem' }}>
            <div style={{ height: '500px', borderRadius: 'var(--border-radius)', overflow: 'hidden', marginBottom: '1rem', position: 'relative' }}>
              <Image 
            ...
            <div style={{ display: 'flex', gap: '1rem', overflowX: 'auto', paddingBottom: '0.5rem' }}>
              {images.map((img: string, i: number) => (
                ...
                >
                  <Image src={img} alt="" width={100} height={70} style={{ objectFit: 'cover' }} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ background: 'var(--surface-1)', padding: '2.5rem', borderRadius: 'var(--border-radius)', marginBottom: '2rem' }}>
            ...
            <p style={{ color: 'var(--text-muted)', lineHeight: '1.8', whiteSpace: 'pre-line' }}>{property.description}</p>
          </div>
          ...
          <div style={{ background: 'var(--surface-1)', padding: '2.5rem', borderRadius: 'var(--border-radius)', marginBottom: '2rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '2rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                <div><div style={{ fontWeight: 700 }}>{property.bedrooms}</div><div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Habitaciones</div></div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ color: 'var(--accent)', background: 'rgba(197, 160, 89, 0.1)', padding: '0.75rem', borderRadius: '50%' }}><Bath size={24} /></div>
                <div><div style={{ fontWeight: 700 }}>{property.bathrooms}</div><div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Baños</div></div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                <div style={{ color: 'var(--accent)', background: 'rgba(197, 160, 89, 0.1)', padding: '0.75rem', borderRadius: '50%' }}><Maximize size={24} /></div>
                <div><div style={{ fontWeight: 700 }}>{property.area} m²</div><div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Área</div></div>
              </div>
            </div>
          </div>
        </section>
        ...
        <aside>
          <div className="glass" style={{ padding: '2rem', borderRadius: 'var(--border-radius)', position: 'sticky', top: '100px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2rem' }}>
              <div style={{ width: '60px', height: '60px', borderRadius: '50%', overflow: 'hidden', border: '2px solid var(--accent)', position: 'relative' }}>
                <Image src={property.owner.avatar || "https://ui-avatars.com/api/?name=" + property.owner.name} alt={property.owner.name} fill style={{ objectFit: 'cover' }} />
              </div>
              <div><div style={{ fontWeight: 700 }}>{property.owner.name}</div><div style={{ fontSize: '0.875rem', color: 'var(--text-muted)' }}>Asesor Elite</div></div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <PremiumButton fullWidth><Phone size={18} /> Ver Teléfono</PremiumButton>
              <PremiumButton fullWidth variant="outline"><Mail size={18} /> Enviar Mensaje</PremiumButton>
            </div>
          </div>
        </aside>
      </div>
